...
uniform float cel_divisions : hint_range(0., 10.) = 2.;
uniform float cel_contrast: hint_range(0., 1.) = .75;
...
void light() {
    ...
    float cel = floor((dot(NORMAL, LIGHT) + 1.)/2. * (cel_divisions+1.)) / (cel_divisions+1.);
    litness = mix(litness, vec3(cel), cel_contrast);
